...
clear all
%% parameters
theta = linspace(0,2*pi,20); % covering ring with step size of pi/10
a=5; % radius of the circuit
dl = [a*cos(theta'),a*sin(theta'),zeros(size(theta'))]; % x,y,z 

I = 1; % filament current [A]
dG = 1e9; % filament max discretization step
n = length(dl(:,1)); % number of filaments on ring, 20
...
%% Plots
figure 
plot3(dl(:,1),dl(:,2),dl(:,3),'.-r', 'LineWidth',2) % plot the ring in red
axis tight
axis square
hold on
title('3D Plot of magnetic field of loop in xy plane centered at origin')
xlabel('X')
ylabel('Y')
zlabel('Z')
